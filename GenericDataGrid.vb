...
Namespace ParaLideres
    ...
    Public Class SimpleDataGrid
        ...
        Public Function RenderToString(Optional ByVal intSectionID As Integer = 0) As String
            ...
            Me.BindDataGrid(_sql_string, intSectionID)
        ...
        End Function
        Public Function RenderToStringSimple() As String
            ...
            Dim mTable As Table = New Table
            Dim mRow As TableRow = New TableRow
            ...
            dgGrid.HeaderStyle.BackColor = colorHeader
            dgGrid.HeaderStyle.ForeColor = Color.White
            dgGrid.HeaderStyle.CssClass = "TABLEHEADER"
            ...
            'define colors for items on grid
            dgGrid.ItemStyle.CssClass = "GEN"
            dgGrid.ItemStyle.BackColor = Color.White 'This is correct!  The back color of the item is the fore color of the grid
            dgGrid.ItemStyle.ForeColor = Color.Black
            ...
            Dim mDR As System.Data.SqlClient.SqlDataReader

            Try

                mContext.Trace.Write("sql: " & _sql_string)

                mDR = ExecReader(_sql_string)
            ...
            Catch ex As Exception
                ...
                mContext.Trace.Warn("GenericDataGrid: Error on Try dgGrid.DataBind(" & _sql_string & ")")
                mContext.Trace.Warn(ex.Source)
                mContext.Trace.Warn(ex.Message)
                mContext.Trace.Warn(ex.ToString)
            ...
            Else
                ...
                mCell.Text = "<p class=GEN>" & _default_message & "</p>"
        ...
        Public Function RenderToStringSimple(ByVal objDataSource As Object) As String
            ...
            Try
                ...
                dgGrid.DataSource = objDataSource
                dgGrid.DataBind()

            Catch ex As Exception

                mContext.Trace.Warn("GenericDataGrid(objdatasourcetry) on bindData<br>" & ex.ToString())
                sb.Append(ex.ToString())
        ...
        Public Sub BindDataGrid(ByVal sqlString As String, Optional ByVal intSectionId As Integer = 0)
            ...
            Try
                ...
                If _index > intRecords Then
                    ...
                    BindDataGrid(_sql_string, intSectionId)
            ...
            For intX = 0 To intPages - 1
                ...
                Else
                    ...
                    strNumericMenu = strNumericMenu & " <a href=" & _page_URL & "?index=" & (intX * _page_size) + 1 & ">" & intX + 1 & "</a> "
            ...
            If intPages > 1 Then
                ...
                Select Case intCurrentPage
                    ...
                    Case Is <= 1
                        ...
                        _strNext = "<a href=" & _page_URL & "?index=" & _index + _page_size & ">Siguiente >></a>"

                    Case Is >= intPages

                        _strPrev = "<a href=" & _page_URL & "?index=" & _index - _page_size & "><< Previo</a> " & strNumericMenu
                        _strNext = "Siguiente >>"
        ...
        Sub dgGrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs)
